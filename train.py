...
def train(hp): 
	...
	f1_score_history = {'train': np.zeros(hp['epochs']), 'valid': np.zeros(hp['epochs'])}
	...
	for epoch in tqdm(range(hp['epochs'])):
		...
		f1_score_history['train'][epoch] = f1
	...
	plt.figure(figsize=(8, 5))
	plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))
	plt.plot(f1_score_history['train'], c='r', label='training')
	plt.ylabel('F1 Score')
	plt.xlabel('Epoch')
	plt.legend(loc='lower right')
	plt.savefig(os.path.join(hp['model_path'], 'f1_score.png'))
	plt.show()
